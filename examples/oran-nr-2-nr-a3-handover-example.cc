...
NS_LOG_COMPONENT_DEFINE("OranNr2NrA3HandoverExample");
 ...
 *
 * The scenario is the same as oran-nr-2-nr-a2-a4-handover-example and
 * oran-nr-2-nr-distance-handover-example: a single NR UE moving back and forth
 * between two NR gNBs, so that the three can be compared directly.
 *
 * The gNBs report to the RIC the 3GPP RRC Measurement Reports that they receive
 * from the UE, and OranLmNr2NrA3RsrpHandover hands over to the strongest
 * neighbour cell in the report.
 *
 * Event A3 is the strongest cell condition: a neighbour became offset better
 * than the primary cell. The whole condition is evaluated in the UE RRC:
 *
 * - The A3 entry inequality, Mn + Ofn + Ocn - Hys > Mp + Ofp + Ocp + Off, is
 ...
 *
 * The Logic Module therefore has no Hysteresis, no A3Offset, and no
 * TimeToTrigger of its own, and holds no per-neighbour dwell state. An Event A3
 * report only exists because the condition held continuously for the whole
 * time-to-trigger, so all the Logic Module does is pick the strongest reported
 * neighbour, exactly as NrA3RsrpHandoverAlgorithm::DoReportUeMeas does.
 *
 ...
 * - `gnb`: the gNBs run the stock ns3::NrA3RsrpHandoverAlgorithm with
 *   `Hysteresis` set to `--hysteresis` and `TimeToTrigger` to
 *   `--timeToTrigger`, and no RIC is instantiated. The algorithm installs the
 *   same Event A3 reporting configuration that the `oran` mode installs by
 *   hand, so both modes see the same Measurement Reports and differ only in
 *   where and when they are acted upon.
 ...
 * modes comparable.
 *
 * Note that the RIC loop adds its own delay on top of the time-to-trigger: the
 * Reporter has to drain, the Report has to reach the RIC, the Logic Module has
 * to be queried, and the Command has to travel back. A RIC-side Event A3 is
 * therefore always slower to act than the gNB-side one on the identical
 * measurements, and that difference is one of the things this example measures.
 *
 ...
 * in dBm. If the run produces no Event A3 reports at all, no neighbour ever got
 * `--hysteresis` dB better than the serving cell for `--timeToTrigger`: read the
 * printed RSRP values around the middle of the UE's travel and lower the margin.
 */
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("speed",
                 ...
                 speed);
    cmd.AddValue("distance", "Distance between the two gNBs in metres", distance);
    ...
    // The UE sweeps a fixed path that overshoots both gNBs by an eighth of their
    // separation on each side, reversing at the ends. Deriving the reversal
    // interval from the speed keeps that path identical as the speed changes, so
    // that a speed sweep varies only how long the UE spends near the cell edge
    // and not where it travels.
    Time interval = Seconds(1.125 * distance / speed);
