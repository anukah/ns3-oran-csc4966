...
NS_LOG_COMPONENT_DEFINE("OranNr2NrCmmHandoverExample");

/**
 * Example that demonstrates the OranCmmHandover conflict mitigation module
 * with NR-to-NR handover commands.
 *
 * Same scenario as oran-nr-2-nr-rsrp-handover-lm-example (1 UE bouncing
 * between 2 gNBs) but with:
 *   - OranCmmHandover instead of OranCmmNoop
 *   - A short LM query interval (1s) to increase the chance that the LM
 *     generates a duplicate handover command while a previous one is still
 *     pending — allowing the CMM to filter it.
 *
 * To verify CMM behaviour, inspect the cmmaction table:
 *   sqlite3 oran-repository.db "SELECT * FROM cmmaction ORDER BY time;"
 *
 * Expected: rows containing "Excluding a pending command" when duplicates
 * are filtered.
 */
