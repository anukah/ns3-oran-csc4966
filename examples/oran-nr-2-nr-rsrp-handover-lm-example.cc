...
/**
 * Example of the O-RAN NR bridge using RSRP-based handover.
 *
 * The scenario: 1 NR UE moves back and forth between 2 NR gNBs.
 * The UE reports location, cell ID, and RSRP/RSRQ measurements to the RIC.
 * The OranLmNr2NrRsrpHandover Logic Module periodically inspects the latest
 * RSRP per neighbour cell and issues an OranCommandNr2NrHandover whenever a
 * neighbour's RSRP exceeds the serving cell's. The gNB-side automatic handover
 * algorithm is disabled (NrNoOpHandoverAlgorithm) so all handovers come from
 * the RIC.
 */
...
main(int argc, char* argv[])
{
    ...
    // NR UE terminators — built manually so we can connect the NrUePhy
    // ReportUeMeasurements trace into the RSRP/RSRQ reporter.
    for (uint32_t idx = 0; idx < ueNodes.GetN(); idx++)
    {
        Ptr<OranReporterLocation> locationReporter = CreateObject<OranReporterLocation>();
        Ptr<OranReporterNrUeCellInfo> cellInfoReporter =
            CreateObject<OranReporterNrUeCellInfo>();
        Ptr<OranReporterNrUeRsrpRsrq> rsrpRsrqReporter =
            CreateObject<OranReporterNrUeRsrpRsrq>();
        Ptr<OranE2NodeTerminatorNrUe> nrUeTerminator =
        ...
        cellInfoReporter->SetAttribute("Terminator", PointerValue(nrUeTerminator));
        cellInfoReporter->SetAttribute(
            "Trigger",
        ...
        rsrpRsrqReporter->SetAttribute("Trigger",
                                       StringValue("ns3::OranReportTriggerPeriodic"));

        // Connect NrUePhy::ReportUeMeasurements → reporter
        for (uint32_t d = 0; d < ueNodes.Get(idx)->GetNDevices(); d++)
        {
            ...
            if (nrUeDev)
            {
                ...
                uePhy->TraceConnectWithoutContext(
                    ...
                    MakeCallback(&OranReporterNrUeRsrpRsrq::ReportRsrpRsrq, rsrpRsrqReporter));
                break;
            }
        }
        ...
        nrUeTerminator->AddReporter(cellInfoReporter);
        nrUeTerminator->AddReporter(rsrpRsrqReporter);

        nrUeTerminator->Attach(ueNodes.Get(idx));
        e2NodeTerminatorsUes.Add(nrUeTerminator);
    }

    // NR gNB terminators — straightforward via OranHelper
